...
def _compute_class_statistics(
    ...
    """Return per-class mean vectors, precision matrix and log-det of covariance."""
    ...
    if sign <= 0:
        log_det = math.log(np.abs(np.linalg.det(covariance)) + 1e-12)
    return class_means, precision, log_det
...
def run_open_set_pipeline(
    ...
    *,
    ...
    class_means, precision, log_det_covariance = _compute_class_statistics(
        train_features,
        train_labels,
        num_classes,
    )
